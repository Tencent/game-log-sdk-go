...
> 说明：
>
> - V3协议的同步性能与V1版本相差如此之大，是因为V3协议需要等待响应，收到响应在认为一个请求结束，而V1版本只管发送，不收响应；
> - V3协议异步UDP与TCP性能相差大是因为UDP发送后无须缓存，接收放也无须拆包，当把批次发送的数据量调大时（每批20条或者10K日志），TCP也能以较高的速率发送数据（QPS：35W）。
