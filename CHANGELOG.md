...
> 说明：建议升级到此版本，修复了关闭时可能出现的goroutine泄漏问题。
...
Bug修复：
- 修复`Client.Close()`关闭顺序导致的goroutine泄漏：原先先停止`netClient`再关闭连接池，若此时仍有`startDial`发起的拨号在途，拨号会在gnet事件循环已退出后卡在`EnrollContext`的连接注册等待上，永久无法返回；现调整为先关闭连接池、等待在途拨号结束（带超时兜底，避免外部`Dialer`未处理超时导致关闭本身被无限阻塞），再停止`netClient`；
- 修复连接池`Put`/`OnConnClosed`与`Close`并发时的入队竞态：原先仅靠`closed`标志和`closeDone`判断是否还能入队，存在`closed.Load()`读到false之后才执行入队、而此时`Close`已经完成等待并退出的窗口，导致命令残留在无人消费的`cmdCh`中；新增`enqueueMu`作为准入门，在同一把锁内完成"关闭状态切换"与"入队登记"，保证`Close`等待开始后不会再有新命令被接纳；

## 0.7.0(2026-07-09)

> 说明：建议升级到此版本，以获得更好的服务质量。

特性：
- 连接池支持注入心跳函数给所有空闲连接发送心跳。

改进：
- worker改为通过连接池发送心跳。

Bug修复：
- 修复V3批次在发送超时进入退避重试后，迟到响应、发送失败回调和关闭流程之间的竞态问题：新增`retryingBatches`/`doneBatches`状态管理，统一通过`doneBatch`清理待确认与重试中的批次，并在响应到达或关闭时安全取消重试定时器，避免已完成或已复用的batch被再次重试/回调。


## 0.6.3(2026-07-06)

> 说明：建议升级到此版本，彻底修复了gnet API的数据竞争问题，运行更稳定。
...
Bug修复：
- 升级gnet包到v2.9.6版本，修复连接会被子进程继承的问题。https://github.com/panjf2000/gnet/pull/743
